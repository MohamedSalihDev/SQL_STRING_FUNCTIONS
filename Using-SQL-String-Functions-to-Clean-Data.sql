...
SELECT
	first_name
	,LENGTH(first_name) AS firs_name_length
FROM
	employees
WHERE
	gender = 'M';
...
-- 2.4: Retrieve a list of the customer group of all customers, which are the
--first two digits of their customer_id.

SELECT 
	LEFT(customer_id, 2) AS customer_group
FROM
	customers;
...
SELECT
	RIGHT(customer_id, 5) AS customer_number
FROM
	customers;
...
SELECT
	customer_id
	,RIGHT(customer_id, LENGTH(customer_id) - 3)
FROM
	customers;
...
SELECT
	UPPER('Courser Guided Project');


-- 3.2: Change Coursera-Guided-Projects to lowercase letters
SELECT
	LOWER('Courser Guided Project');
...
SELECT
	...
	,last_name
	,gender
	,REPLACE(gender, 'M', 'Male') AS emp_gender
FROM
	employees;
...
SELECT
	...
	,last_name
	,gender
	,REPLACE(gender, 'F', 'Female') AS emp_gender
FROM
	employees;
...
SELECT
	bracket_cust_id
	,TRIM(bracket_cust_id, '()') AS cleaned_customer_id
FROM
	customers;
...
SELECT
	customer_name || ' is ' || age || ' years old' AS desc_age
FROM
	customers
ORDER BY 
	age DESC;
...
SELECT
	customer_name
	,CONCAT(customer_name, ' is ', age, ' years old')
FROM
	customers;
...
SELECT
	customer_id
	,customer_name
	,SUBSTRING(customer_id, 1, 2) AS customer_group
FROM
	customers;
...
SELECT
	last_name
	,SUBSTRING(TO_CHAR(birth_date, 'YYYY-MM-DD'), 1, 4) AS employee_birth_year
FROM
	employees;
...
SELECT
	last_name
	,SUBSTRING(CAST(birth_date AS CHAR(10)), 1, 4) AS employee_birth_year
FROM
	employees;
...
SELECT
	customer_id
	,STRING_AGG(product_id, ', ') AS all_products_ordered
FROM
	sales
GROUP BY 
	customer_id;
...
SELECT
	dept_no
	,dept_name
	,COALESCE(dept_number, department_name) AS dept
FROM
	dept_dup
ORDER BY
	dept_no;
...
SELECT
	customer_id
	,COALESCE(country, city, state, 'No Address') AS location
FROM
	customers;
